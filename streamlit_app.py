...
if st.button("Generate Visualization"):
    if not openai_api_key:
        ...
        with st.status("Fetching embeddings...", expanded=True) as status:
            try:
                embeddings = np.array([get_embedding(word) for word in words_list])
                ...
                status.update(label="Embeddings fetched and visualization complete!", state="complete", expanded=True)

            except Exception as e:
                status.update(label=f"Error fetching embeddings: {e}", state="error", expanded=False)